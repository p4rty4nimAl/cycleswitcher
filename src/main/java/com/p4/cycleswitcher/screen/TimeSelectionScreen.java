...
public class TimeSelectionScreen
        extends Screen {
    ...
    private static final Text SELECT_NEXT_TEXT = new TranslatableText("debug.times.select_next", new TranslatableText("debug.times.press_f6").formatted(Formatting.AQUA));
    ...
    protected void init() {
        super.init();
        this.weather = this.currentWeather.isPresent() ? this.currentWeather : WeatherSelection.of(client.world.getTimeOfDay() < 6000 ? WeatherSelection.DAY : client.world.getTimeOfDay() < 13000 ? WeatherSelection.NOON : client.world.getTimeOfDay() < 18000 ? WeatherSelection.NIGHT : WeatherSelection.MIDNIGHT);
    ...
    private static void apply(MinecraftClient client, Optional<WeatherSelection> weather) {
        ...
        //Optional<WeatherSelection> optional = WeatherSelection.of(client.world.getTimeOfDay() < 6000 ? WeatherSelection.DAY : client.world.getTimeOfDay() < 13000 ? WeatherSelection.NOON : client.world.getTimeOfDay() < 18000 ? WeatherSelection.NIGHT : WeatherSelection.MIDNIGHT);
        WeatherSelection weatherSelection = weather.get();
        if (client.player.hasPermissionLevel(2)) {///*optional.isPresent() &&*/  && weatherSelection != optional.get() && weatherSelection != lastWeather) {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (keyCode == GLFW.GLFW_KEY_F6 && this.weather.isPresent()) {
    ...
    enum WeatherSelection {
        DAY(new TranslatableText("time.day"), "/time set day", new ItemStack(Blocks.ORANGE_CONCRETE)),
        NOON(new TranslatableText("time.noon"), "/time set noon", new ItemStack(Items.SUNFLOWER)),
        NIGHT(new TranslatableText("time.night"), "/time set night", new ItemStack(Items.INK_SAC)),
        MIDNIGHT(new TranslatableText("time.midnight"), "/time set midnight", new ItemStack(Items.BLACK_DYE));
    ...
    public class ButtonWidget
        ...
        public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
            ...
            this.weather.renderIcon(TimeSelectionScreen.this.itemRenderer, this.x + 5, this.y + 5);
            if (this.selected) {
